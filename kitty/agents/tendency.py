...
LEVEL_VALUES: dict[str, dict[int, float]] = {
    # 익절 목표 수익률 (%) — 목표를 높여 수익 추적 기간 확대, 손익비 개선
    "take_profit": {1: 5.0, 2: 8.0, 3: 12.0, 4: 18.0, 5: 25.0, 6: 35.0},
    # 손절 기준 손실률 (%) — 임계치를 촘촘하게 조여 손실 폭 최소화
    "stop_loss":   {1: -1.5, 2: -2.5, 3: -3.5, 4: -5.0, 5: -7.5, 6: -10.0},
    # 최소 현금 비중 (0~1)
...
PRESETS: dict[str, dict[str, int]] = {
    # 목표 높게·손절 빠르게, 집중 투자, 낮은 현금 — R:R 3.2:1
    "aggressive": {
        "take_profit": 2,  # +8.0%
        "stop_loss":   2,  # -2.5%  (손익비 1:3.2)
        "cash":        2,  # 18%
        "max_weight":  2,  # 28%
        "entry":       2,  # +4.5%
    },
    # 균형 잡힌 기준, 적절한 분산 — R:R 3.4:1
    "balanced": {
        "take_profit": 3,  # +12.0%
        "stop_loss":   3,  # -3.5%  (손익비 1:3.4)
        "cash":        4,  # 35%
        "max_weight":  4,  # 17%
        "entry":       3,  # +3%
    },
    # 충분한 수익 대기, 높은 현금, 분산 투자 — R:R 7.1:1
    "conservative": {
        "take_profit": 5,  # +25.0%
        "stop_loss":   3,  # -3.5%  (손익비 1:7.1)
        "cash":        5,  # 48%
        "max_weight":  5,  # 13%
        "entry":       5,  # +1%
    },
...
def _build_directive(levels: dict[str, int], rationale: str = "") -> str:
    ...
    soft_sl = round(sl * 0.5, 2)        # 소프트 스탑: 하드 손절의 50%
    trail_trigger = round(tp * 0.5, 1)  # 트레일링 스탑 발동: 익절 목표의 50%
...
[손실 최소화 기술 지침 — 반드시 적용]
① 소프트 스탑 (조기 경고): 평가손실 -{soft_sl}% 도달 시 PARTIAL_SELL 검토 시작.
   - 섹터 bullish + 단기 조정 명확 → HOLD 허용, 단 다음 사이클에서 재평가 필수
   - 섹터 neutral/bearish → 즉시 PARTIAL_SELL 실행 (하드 손절 대기 금지)
② 하드 스탑 (손절 실행): 평가손실 -{sl}% 초과 시 무조건 PARTIAL_SELL(~50%). 예외 없음.
③ 비상 스탑 (전량 정리): 평가손실 -{round(sl*2, 1)}% 초과(손절 기준 2배) 또는 하한가 근접 → 전량 SELL.
④ 트레일링 스탑 (수익 보전): 평가수익이 한때 +{trail_trigger}% 이상 달성 후 현재 수익이 +{round(trail_trigger*0.4, 1)}% 미만으로 반납 → PARTIAL_SELL 실행.
   - 모멘텀이 꺾이면 수익을 확보하라. 다시 오르면 재진입 기회 있음.
⑤ 거래량 모멘텀 이탈: 당일 등락률이 -1.5% 이하이면서 섹터가 neutral/bearish → 손절 기준 미달이라도 PARTIAL_SELL 고려.
⑥ 손익비 원칙: 신규 매수 종목은 예상 TP/SL 비율이 2.5:1 이상인 종목만 선택."""
...
■ 익절 (take_profit): 수익 추적 목표 — 높을수록 수익을 더 오래 추적
  L1(+5%) L2(+8%) L3(+12%) L4(+18%) L5(+25%) L6(+35%)

■ 손절 (stop_loss): 손실 차단 속도 — 낮을수록(L1) 더 빠르게 손절
  L1(-1.5%) L2(-2.5%) L3(-3.5%) L4(-5%) L5(-7.5%) L6(-10%)
  ※ L1이 가장 빠른 손절, L6이 가장 느린 손절
  ※ 손절 기준의 50% 지점을 소프트 스탑으로 활용 (조기 경고 시스템)
...
에이전트 점수는 0~100점 척도입니다.

에이전트 점수 기반 조정 방향:
- 자산운용가·종목발굴가 점수 낮음(40↓) → 익절(L-), 손절(L-) 조정 (더 공격적)
- 종목평가가·섹터분석가 점수 낮음(40↓) → 현금(L+), 진입기준(L+) 조정 (더 보수적)
- 매도실행가 점수 낮음(40↓) → 손절(L-) 조정 (더 빠른 손절)
- 매수실행가 점수 낮음(40↓) → 현금(L+), 진입기준(L+) 조정 (더 선별적 진입)
- 전반적 고점수(70↑) → 현재 레벨 유지 또는 소폭 공격적 조정
- 전반적 저점수(40↓) → 각 차원 1~2 레벨 보수적으로 이동
"""
...
점수는 0~100점 척도입니다.
- 자산운용가·종목발굴가 점수 낮음(40↓) → take_profit, stop_loss 레벨 낮추기 (더 공격적)
- 섹터분석가·종목평가가 점수 낮음(40↓) → cash, entry 레벨 높이기 (더 보수적)
- 매도실행가 점수 낮음(40↓) → stop_loss 레벨 낮추기 (더 빠른 손절)
- 전반 고점수(70↑) → 현재 레벨 유지 또는 소폭 낮추기 (공격적)
- 전반 저점수(40↓) → 각 레벨 1~2 올리기 (보수적 이동)
