...
08:50  사이클 시작 (장 시작 10분 전부터 분석)
  ...
  │      5차원 × 6단계 레벨 조합으로 지침 문자열 생성
  │      → 종목평가가 · 종목발굴가 · 자산운용가 프롬프트에 주입
  │
  ├─ [1] 🔭 섹터분석가 (SectorAnalystAgent)
  │      실시간 시장 데이터 (바로미터 ETF 시세 + 거래량 상위 20종목) 기반 분석
  │      → 유망/위험 섹터 판단 + 섹터별 후보 종목 코드 제시
  │      (추측·뉴스 기반 판단 금지, 실측 수치만 사용)
  │
  ├─ 📡 시세 조회 (후보 종목 + 보유 종목 전체 + 거래량 상위 20종목)
  │
...
15:35  📈 성과 평가 (장 마감 5분 후, 하루 1회)
       ...
       → 에이전트별 정량 점수(0~100) 계산 + 의사결정 상세 기록
       → AI 피드백 생성 (summary + good_pattern + improvement)
       → feedback/*.json 저장 (최근 14일, 다음 사이클부터 system_prompt에 자동 반영)
       → TendencyAgent.update_strategy() — 내일 차원별 레벨 AI 조정 → tendency_state.json 저장
       → Telegram으로 평가 결과 전송
```
...
| 🎯 **투자 성향** | 5차원 × 6단계 레벨로 익절·손절·현금·집중도·진입기준 독립 조정. 장 마감 후 AI가 자동 갱신 |
| 📡 **자율 종목 선정** | 고정 watchlist 없음. 실시간 시장 데이터 기반 섹터 분석으로 후보 발굴 |
| 💰 **잔고 기반 배분** | 투자성향 지침의 현금 유보 비율 준수, Rotation 전략 자동 실행 |
| ⚡ **스마트 주문** | 분할 주문 + 지정가 → 취소 → 시장가 전환 (최대 3회) |
| 🛑 **손절 즉시 실행** | HIGH priority 시장가 주문, 어떠한 지연도 없음 |
| 🔁 **에이전트 자기개선** | 장 마감 후 점수(0~100) + 의사결정 상세 + good_pattern 누적 → 다음날 system_prompt 자동 반영 |
...
**장 마감 후 `update_strategy()`를 통해 AI가 내일 레벨을 자동 조정합니다 (1사이클 ±2 제한).**

### 5차원 × 6단계 레벨

| 차원 | L1 (최공격) | L3 | L4 (균형) | L6 (최보수) |
|------|------------|-----|----------|------------|
| `take_profit` 익절 | +5% | +10% | +15% | +25% |
| `stop_loss` 손절 | -2% | -4% | -6% | -10% |
| `cash` 현금 유보 | 10% | 20% | 30% | 55% |
| `max_weight` 종목 최대 비중 | 35% | 25% | 20% | 12% |
| `entry` 과열 진입 기준 | +8% | +4% | +2% | -2% |

초기 레벨: 모든 차원 **L2 (공격적)**. 상태는 `logs/tendency_state.json`에 영속 저장됩니다.

프리셋:
- `aggressive`: 모든 차원 L2
- `balanced`: 대부분 L4
- `conservative`: L5/L3 혼합

모니터 대시보드 성적표 탭 상단 성향 카드에서 5개 차원 레벨 배지로 현재 성향을 확인할 수 있습니다.
...
| 에이전트 | 평가 기준 | 점수 |
|----------|-----------|------|
| 🔭 섹터분석가 | bullish/bearish 예측 vs 실제 섹터 등락 | 적중률 × 100 |
| 🔍 종목발굴가 | 추천 종목 당일 수익률 평균 | 구간별 20~90점 |
| 📊 종목평가가 | HOLD/BUY_MORE/SELL 판단 정확도 | 정확도 × 100 |
| 💼 자산운용가 | 최종 주문 방향성 (매수→상승, 매도→하락) | 구간별 20~90점 |
| 🟢 매수실행가 | 체결가 vs EOD (저가 매수 효율) | 구간별 30~90점, 체결 0건 시 10점 |
| 🔴 매도실행가 | 체결가 vs EOD (고가 매도 효율) | 구간별 30~90점, 체결 0건 시 10점 |

### 반영 방식

```
평가 완료
  → 의사결정 상세 기록 (✓/✗ per decision) → AI 피드백 생성
  → feedback/{에이전트명}.json 누적 저장 (최근 14일)
     · summary (100자), good_pattern (유지할 성공 패턴 80자), improvement (200자)
  → 각 에이전트 system_prompt 끝에 최근 5일 피드백 자동 주입
     · 점수 추이 (최근 7일) + 📈/📉/➡️ 추이 아이콘
     · [최우선 개선 과제] (최근 3일 중복 제거)
  → TendencyAgent.update_strategy() — 각 차원 레벨 AI 조정
  → 다음 사이클부터 AI가 과거 실수와 성공 패턴을 참고해 판단 개선
```
...
| 항목 | 기준 |
|------|------|
| 💵 사이클당 투입 한도 | 투자성향 지침의 현금 유보 비율 준수 (기본 L2: 10%) |
| 📦 종목당 최대 비중 | 투자성향 지침의 `max_weight` 레벨 (기본 L2: 35%) |
| 🔻 손절 | 투자성향 지침의 `stop_loss` 레벨 (기본 L2: -3%), HIGH priority 즉시 시장가 |
| 🔺 익절 | 투자성향 지침의 `take_profit` 레벨 (기본 L2: +5%), PARTIAL_SELL 우선 |
| ⛔ 유동성 부족 | 거래량 10만주 미만 또는 거래대금 10억 미만 종목 매수 금지 |
...
| 화면 | 내용 |
|------|------|
| 🤖 **성적표** (기본) | 투자 성향 카드 (종합 평가 리포트 more/less 접기) + 포트폴리오 현황 + 에이전트 점수 카드 + 7일 히트맵 |
...
| `logs/tendency_state.json` | 투자성향 5차원 레벨 상태 (EC2 재시작에도 유지) |
| `feedback/{에이전트명}.json` | 성과 피드백 누적 (최근 14일, summary + good_pattern + improvement) |
