...
| 📒 매매일지 | 현재 뷰·모드 기준 자동 필터 — Live/Paper 거래 분리 표시 |
| 🛡️ 손실 최소화 | 소프트 스탑(하드 손절 50%) + 트레일링 스탑 + 손익비(R:R) 2.5:1 필터 |
| 📡 자율 종목 선정 | 고정 watchlist 없음. 실시간 시장 데이터 기반 |
| ⚡ 스마트 주문 | 분할 주문 + 지정가→시장가 전환 (한국 5주/미국 10주 기준) |
| 🔁 자기개선 | 누적 피드백 시스템 — 반복 이슈 자동 탐지 + 검증된 패턴 강화 → 매 사이클 반영 |
| 🌐 AI 백엔드 | Anthropic Claude / OpenAI GPT / Google Gemini |
| 💬 Telegram | 21개 명령어 원격 제어 + 모니터링 |
| 📊 대시보드 | kitty-monitor — 🐱/🌙 뷰 전환, 성적표·매매일지, Agent 관리, 시스템(투자모드·LLM) |
| 🔀 투자 모드 | Paper ↔ Live 런타임 전환, 컨테이너 재시작 후 모드 자동 복원 |
...
Kitty는 단순 손절 기준 외에 다층 방어 구조로 손실 폭을 제한합니다.

| 단계 | 트리거 | 동작 |
|------|--------|------|
| ① 소프트 스탑 | 하드 손절 기준의 50% 손실 | 섹터 약세이면 PARTIAL_SELL 즉시 실행 |
| ② 하드 스탑 | 손절 임계치 초과 | PARTIAL_SELL ~50% 무조건 실행 |
| ③ 비상 스탑 | 손절 기준 2배 초과 | 전량 SELL |
| ④ 트레일링 스탑 | TP 50% 달성 후 반납 시 | PARTIAL_SELL로 수익 보전 |
| ⑤ 거래량 이탈 | 당일 -1.5%↓ + 섹터 약세 | 손절 전이라도 PARTIAL_SELL 고려 |
| ⑥ 자본보호 모드 | 포트폴리오 합산 -3% 이상 | 신규 매수 전면 중단, 손절 우선 |
| ⑦ 매수 전 잔고 검증 | 주문 가능 금액 실시간 확인 | 잔고 부족 시 수량 자동 조정 또는 스킵 |
| ⑧ 매도 전 보유 검증 | 보유수량 실시간 확인 | 초과 매도 방지 + 중복 매도 차단 |

**R:R 필터**: 신규 매수 종목은 예상 손익비(TP÷SL) **2.5:1 이상**인 경우에만 승인합니다.

### 투자 성향 레벨 값 (v2.2.0 기준)

| 차원 | L1 (공격) | L2 | L3 | L4 | L5 | L6 (보수) |
|------|-----------|-----|-----|-----|-----|-----------|
| 익절 | +5% | **+8%** | +12% | +18% | +25% | +35% |
| 손절 | -1.5% | **-2.5%** | -3.5% | -5% | -7.5% | -10% |
| R:R (L2) | | **3.2:1** | | | | |

기본 프리셋 `aggressive` = L2 기준: 익절 **+8%**, 손절 **-2.5%**, R:R **3.2:1**
