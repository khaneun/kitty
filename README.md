...
08:50  사이클 시작 (장 시작 10분 전부터 분석)
  │
  ├─ [0] 투자성향관리자 (TendencyAgent)          ← AI 호출 없음, 즉시 실행
  │      현재 성향 프로필(공격적/균형/보수적)에서 지침 문자열 생성
...
| 모니터링 대시보드 | kitty-monitor (포트 8080) — 성적표 메인 + 관리 서브탭(상태·에러·토큰) + FAB 채팅 |
| 포트폴리오 스냅샷 | 매 사이클 후 portfolio_snapshot.json 저장, 모니터 성적표 탭에 실시간 표시 |
| 토큰 사용량 추적 | AI 호출마다 token_usage/에 기록, 모델별 USD 비용 자동 추산 |
| 에이전트 채팅 | 모니터 우하단 FAB(💬) → 슬라이드업 팝업으로 각 에이전트에 직접 질문 |
| GNB 모드 셀렉터 | 모니터 헤더에서 paper/live 전환, 포트폴리오 현황과 연동 |
| 투자 성향 에이전트 | 공격적/균형/보수적 프로필로 에이전트 판단 임계값 자동 조정, 성적표에 성향 카드 표시 |
...
│   └── tendency.py            # [7] 투자성향관리자 (공격적/균형/보수적 성향 지침 주입, AI 호출 없음)
├── broker/
...
진입 시 **🤖 성적표**가 기본 화면으로 표시됩니다.

| 화면 | 주요 정보 |
|------|----------|
| 🤖 성적표 (메인) | 투자 성향 카드 + 포트폴리오 현황(총평가·손익·현금·종목 테이블) + 에이전트 점수 카드 + 히트맵 |
| ⚙️ 관리 → 🏥 상태 | ok/warning/critical 배지, 1시간 에러 건수, 마지막 로그 시각, 최근 에러 5건 |
| ⚙️ 관리 → 📋 에러 | 14일 에러 추이 차트, 날짜·레벨·키워드 필터, 로그 전문 조회 |
| ⚙️ 관리 → 🔢 토큰 | 오늘 토큰·비용, 에이전트별 누적 바 차트, 14일 일별 추이 |
| 💬 FAB (우하단) | 에이전트 선택 후 자유 질문 — 슬라이드업 팝업, 최근 판단 컨텍스트 기반 답변 |
